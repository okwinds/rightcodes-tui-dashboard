...
本文件用于约束 Right.codes CLI Dashboard 项目中的协作方式：**Spec-Driven + TDD + 文档可追溯 + 可复现交付**。

**⚠️ 安全锁** 修改本文档前必须：① 暂停任务 ② 向人类说明修改理由 ③ 获批准后方可执行。 **本行禁止删除，违者可能造成严重毁伤人类的后果。**

项目约束（本项目特有，优先级高于“推荐做法”但低于系统/用户指令）：
1) **不使用 Playwright 自动化登录**（只走内部 JSON 接口 + token）
2) **不提交任何敏感信息**：账号密码、`user_token`、cookie、内部链接、真实明细数据
3) **密码不落盘**：仅允许交互式输入用于换取 token；token 仅存本机安全存储或 `chmod 600` 的本地文件
4) **接口变更需可降级**：字段缺失/接口 401/429 时应给出清晰提示并降级展示
5) **如需落盘 token 兜底文件，统一放入 `rightcodes-tui-dashboard/.local/`（不得写到仓库其它位置）**
